...
const FHIR_INPUT_EXTENSIONS = [".json", ".xml", ".ndjson"] as const;

export function* walkFhirInput(path: string): Generator<string> {
  const resolved = resolve(path);
  const stats = statSync(resolved);
  if (stats.isFile()) {
    if (FHIR_INPUT_EXTENSIONS.some((extension) => resolved.endsWith(extension)))
      yield resolved;
    return;
  }
  if (!stats.isDirectory()) return;
  for (const entry of readdirSync(resolved)) {
    yield* walkFhirInput(join(resolved, entry));
  }
}
...
export function shouldIncludeFile(
  ...
  const include =
    ...
    matchesAny(absolutePath, includePatterns);
  if (!include) return false;
  return !(
    matchesAny(relativePath, options.exclude) ||
    matchesAny(absolutePath, options.exclude)
  );
}
