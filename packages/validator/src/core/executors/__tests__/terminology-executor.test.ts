...
describe("TerminologyExecutor", () => {
  ...
  describe("validate", () => {
    ...
    it("skips binding validation for primitive sidecar-only values", async () => {
      ...
      expect(validateBindingSpy).not.toHaveBeenCalled();
      expect(
        issues.some((issue) => issue.code === "binding-required-missing"),
      ).toBe(false);
    ...
    it("keeps global CodeSystem display hygiene as a warning on the display path", async () => {
      ...
      validatorInstance.validateCodeInCodeSystem.mockResolvedValue({
        ...
        message:
          "Wrong Display Name 'Urine microscopy (procedure)' for http://snomed.info/sct#394712000",
        issues: [
          {
            severity: "error",
    ...
    it("suppresses CodeSystem display mismatches that differ only by case or spacing", async () => {
      ...
      mockContext.getValueAtPath = (resource: any, path: string) => {
        if (path === "MedicationRequest.medication[x]")
    ...
    it("keeps coded value display mismatches as warnings", async () => {
      ...
      validatorInstance.validateCodeInCodeSystem.mockResolvedValue({
        ...
        reason: "display-mismatch",
        message:
          "Wrong Display Name 'Urine leukocyte test' for http://snomed.info/sct#394712000",
        issues: [
          {
            severity: "error",
            code: "invalid-display",
    ...
    it("keeps inactive CodeSystem warnings even when the terminology server returns result=true", async () => {
      ...
      validatorInstance.validateCodeInCodeSystem.mockResolvedValue({
        ...
        inactive: true,
        message:
          "The concept '15777000' has a status of inactive and its use should be reviewed",
        issues: [
          {
            severity: "warning",
            code: "code-comment",
      ...
      mockContext.getValueAtPath = (resource: any, path: string) => {
        ...
        if (path === "Practitioner.identifier.use") {
          return resource.identifier
            .map((identifier: any) => identifier.use)
            .filter(Boolean);
        }
        return undefined;
      };
      ...
      mockStructureDef.snapshot!.element = [
        ...
        } as ElementDefinition,
        {
          ...
          binding: {
            strength: "required",
            valueSet:
              "http://example.org/fhir/ValueSet/patho-section-types-loinc",
          },
        } as ElementDefinition,
      ];
      mockContext.resource = {
    ...
    it("emits indexed UCUM Quantity paths without duplicate hygiene issues", async () => {
      ...
      const issues = await executor.validate(mockContext);
      const invalidUcumIssues = issues.filter(
        (issue) => issue.code === "terminology-code-invalid",
      );

      expect(invalidUcumIssues).toHaveLength(2);
      expect(invalidUcumIssues).toContainEqual(
        expect.objectContaining({
      ...
      expect(invalidUcumIssues).not.toContainEqual(
        expect.objectContaining({
    ...
    it.each([
      ...
      "adds common UCUM correction hints for %s",
      (invalidCode, suggestedCode, suggestedDisplay) => {
      ...
      expect(issues).toContainEqual(
        expect.objectContaining({
          ...
          details: expect.objectContaining({
            code: "ORIGINAL",
            fieldPath:
              "ImagingStudy.series[0].extension[0].extension[0].valueCodeableConcept.coding[1]",
          }),
        }),
      );
    });
    ...
    it("recognizes standard external and national IG CodeSystem URLs", async () => {
      ...
      mockContext.getValueAtPath = (resource: any, path: string) => {
        ...
        if (path === "Patient.communication.language")
          return resource.communication[0].language;
        if (path === "Patient.identifier.type")
          return resource.identifier[0].type;
        return undefined;
      };
      ...
      expect(
        issues.filter(
          (issue) => issue.code === "terminology-codesystem-unresolvable",
        ),
      ).toHaveLength(0);
    });
  });
